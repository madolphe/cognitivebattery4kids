  ...
  if (Time.activetime_block < time_stimduration) {
    ...
    if (flag_practice == true) {
      if (Params.array_stim[Params.ind_stimcond] == 0) {
        Imgs_filler_tutorial[Params.trial_stimind[Params.ind_stimcond]].resize(
          size_rescale,
          size_rescale,
        );
        image(
          Imgs_filler_tutorial[Params.trial_stimind[Params.ind_stimcond]],
          Pos.center_x - size_rescale / 2,
          Pos.center_y - size_rescale / 2,
        );
      } else if (Params.array_stim[Params.ind_stimcond] == 1) {
        Imgs_targ_tutorial[Params.trial_stimind[Params.ind_stimcond]].resize(
          size_rescale,
          size_rescale,
        );
        image(
          Imgs_targ_tutorial[Params.trial_stimind[Params.ind_stimcond]],
          Pos.center_x - size_rescale / 2,
          Pos.center_y - size_rescale / 2,
        );
      ...
      if (Params.array_stim[Params.ind_stimcond] == 0) {
        Imgs_filler[Params.trial_stimind[Params.ind_stimcond]].resize(
          size_rescale,
          size_rescale,
        );
        image(
          Imgs_filler[Params.trial_stimind[Params.ind_stimcond]],
          Pos.center_x - size_rescale / 2,
          Pos.center_y - size_rescale / 2,
        );
      } else if (Params.array_stim[Params.ind_stimcond] == 1) {
        Imgs_targ[Params.trial_stimind[Params.ind_stimcond]].resize(
          size_rescale,
          size_rescale,
        );
        image(
          Imgs_targ[Params.trial_stimind[Params.ind_stimcond]],
          Pos.center_x - size_rescale / 2,
          Pos.center_y - size_rescale / 2,
        );
      } else if (Params.array_stim[Params.ind_stimcond] == 2) {
...
function scene_feedback() {
  ...
  if (Time.activetime_block < time_feedback) {
    if (Params.tmp_res_ob == 1) {
      push();
      save_img.resize(size_rescale_save, size_rescale_save);
      image(
        save_img,
        Pos.center_x - save_img.width / 2,
        Pos.center_y - save_img.width / 2,
      );
      pop();
    }
  } else {
