...
let pos_title_y = Pos.center_y - Math.round(0.2 * Pos.canvas_height);
...
let x_next =
  Pos.center_x + Math.round(0.14 * Pos.canvas_height) - size_next_w / 2; //in pixel
let y_next =
  Pos.center_y + Math.round(0.3 * Pos.canvas_height) - size_next_h / 2; //in pixel
...
let x_previous =
  Pos.center_x - Math.round(0.14 * Pos.canvas_height) - size_previous_w / 2; //in pixel
let y_previous =
  Pos.center_y + Math.round(0.3 * Pos.canvas_height) - size_previous_h / 2; //in pixel
...
let pos_bubble_y2 = Pos.center_y + Math.round(0.15 * Pos.canvas_height);
...
let shift_imageposition = Math.round(0.15 * Pos.canvas_height);
let shift_imageposition_save = Math.round(0.1 * Pos.canvas_height);
...
let y_start =
  Pos.center_y - Math.round(0.1 * Pos.canvas_height) + size_start_h / 2; //in pixel
let size_start_text = Math.round(0.02 * Pos.canvas_height);

//tutorial specific dataset.
let fname_target_tutorial =
  "static/images/pre-post-imgs/list_img_target_tutorial.csv";
let fname_filler_tutorial =
  "static/images/pre-post-imgs/list_img_filler_tutorial.csv";
let num_targetlist_tutorial = 7;
let num_fillerlist_tutorial = 6;
let Imgs_targ_tutorial = [];
let Imgs_filler_tutorial = [];
let array_stim_tutorial = [
  1, 2, 0, 1, 1, 0, 0, 2, 0, 2, 0, 1, 0, 1, 1, 2, 2, 2, 0,
];
let trial_stimind_tutorial = [
  1, 1, 0, 2, 3, 1, 2, 3, 3, 2, 4, 4, 5, 5, 6, 5, 6, 4, 6,
];
let prac_num_rep = 12;
